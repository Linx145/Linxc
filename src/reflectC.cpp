...
CXChildVisitResult CursorVisitor(CXCursor current, CXCursor parent, CXClientData client_data)
{
    ...
    if (current.kind == CXCursor_StructDecl || current.kind == CXCursor_EnumDecl || current.kind == CXCursor_FunctionDecl)
    {
...
LinxcParsedFile Linxc_ReflectC(LinxcNamespace* globalNamespace, IAllocator *allocator, string fileFullName)
{
    ...
    const char** args = (const char**)allocator->Allocate(sizeof(void*));
    args[0] = "-D CIMGUI_DEFINE_ENUMS_AND_STRUCTS";
	CXTranslationUnit tu = clang_parseTranslationUnit(index, fileFullName.buffer, args, 1, NULL, 0, CXTranslationUnit_SkipFunctionBodies); //
	...
	if (tu != NULL)
	{
        ...
        for (usize i = 0; i < root.body.count; i++)
        {
            ...
            if (cursor->kind == CXCursor_StructDecl)// && cursor->body.count > 0)
            {
                ...
                //string str = root.body.Get(i)->ToString(allocator, 0);
                //printf("%s\n", str.buffer);
                //str.deinit();
            }
        }
